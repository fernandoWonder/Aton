...
using Aton.Views.Configs;
using Aton.Views.Comercial.Documents;
using Aton.Views.PDV;
using Aton.Views.Report;
...
namespace Aton.Views
{
    public partial class Main : Form
    {
        ...
        FrmProductTransition frmStockOut;
        FrmProductTransition frmStockTrans;
        ...
        {
            if (isFormInactive(frmStockOut))
            {
                frmStockOut = new FrmProductTransition(ConstructStockControl.productOut);
                frmStockOut.MdiParent = this;
                frmStockOut.Show();
            }
        }
             Armazens frm ;
        ...
        private void barButtonItem7_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (UserCurrent.getCurrentUser().FlagAdmin == true)
            {
                ...
                {
                    if (_storageController.ListAllAsNoTracking().Count > 0)
                    {
                        if (isFormInactive(frmTrans))
                        {
                            activeForm(frmTrans = new FrmProductTransition(ConstructStockControl.productEntry));
                        }
                    }
                    else
                    {
        ...
        {
            if (!(new StorageController().ListAllAsNoTracking().Count > 1))
                MessageBox.Show("Não pode fazer fazer transferência, pois tem apenas um armazem...");
            else if (isFormInactive(frmStockTrans))
            {
                activeForm(frmStockTrans = new FrmProductTransition(ConstructStockControl.transference));
            }
        }
    }
}
