...
using Aton.Views.General.product.StockUI;
...
namespace Aton.Views.Save
{
    public partial class FrmProdutoView : DevExpress.XtraEditors.XtraForm
    {
        ...
        FrmProductTransition _frmProductTransition;
        ...
        const int constructFrmTransition = 6;
        ...
        public FrmProdutoView(FrmProductTransition _frmProducTransition)
        {
            InitializeComponent();
            toGridControlProducts();
            this._frmProductTransition = _frmProducTransition;
            constructorAtivo = constructFrmTransition;
        }
      ...
      /*  public void toComboArmazem()
       ...
       ProductController _prodController = new ProductController();
        ...
        private void gridView1_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            ...
            indexRowGrid = e.RowHandle;
            var valorCelula = gridViewProducts.GetRowCellValue(indexRowGrid, gridViewProducts.Columns["Id_Stock"]);
            //MessageBox.Show("Id_Stock = " + valorCelula.ToString());
            if (constructorAtivo == ContrutorInvoiceAddProduct)
            {
                valorCelula = gridViewProducts.GetRowCellValue(indexRowGrid, gridViewProducts.Columns["Id_Stock"]);
                _frmInvoice.addToList(Convert.ToInt16(valorCelula));
                Close();
            }else if(constructorAtivo == ContrutorProduct_AddProductComposition)
            {
                valorCelula = gridViewProducts.GetRowCellValue(indexRowGrid, gridViewProducts.Columns["Id"]);
            ...
            else if (constructorAtivo == construtorPDVRestaurant)
            {
                ...
                Close();
            }else if(constructorAtivo == constructFrmTransition)
            {
                valorCelula = gridViewProducts.GetRowCellValue(indexRowGrid, gridViewProducts.Columns["Id_Stock"]);
                _frmProductTransition.addProductGrid(Convert.ToInt16(valorCelula));
                Close();
            }
        ...
        {
            // int rowHandle = gridViewProducts.GetSelectedRows()[0];
           // MessageBox.Show(gridViewProducts.GetRowCellValue(rowHandle, gridViewProducts.Columns["Id_Stock"]).ToString());
        }
    }
}
